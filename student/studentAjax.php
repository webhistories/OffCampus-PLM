<?php
	...
	function compute($connect){
		...
		//$disp_amount 
		//$connect = mysqli_connect('localhost', 'root', 'password', 'gp_local');
		$additional = 0;
		...
		// if($totalUnits[0] == null){
		// 	$removeAssessmentSql = mysqli_query($connect, "DELETE FROM assessment_student WHERE student_id = $studentid AND $aysem = $currentsem"); exit;
		// }
		...
		if ($assessmentExistNumber == 0) {
			...
			$UpdateAssessmentSql = mysqli_query($connect, "UPDATE assessment_student SET total_amount = '$totalFees',tuition_amount='$tuitionFee',misc_amount='$misc[0]', other_amount='$otherFee', units='$totalUnits[0]'");
		}
